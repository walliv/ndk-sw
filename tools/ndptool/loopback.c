...
static int ndp_mode_loopback_prepare(struct ndp_tool_params *p)
{
	...
	p->rx = ndp_open_rx_queue_ext(p->dev, p->queue_index, (p->use_userspace_flag || getenv("NFB_NDP_RX_USERSPACE")) ? NDP_OPEN_FLAG_USERSPACE : 0);
	if (p->rx == NULL) {
		warnx("ndp_open_rx_queue(%d) failed.", p->queue_index);
		goto err_ndp_open_rx;
	}

	p->tx = ndp_open_tx_queue_ext(p->dev, p->queue_index, (p->use_userspace_flag || getenv("NFB_NDP_TX_USERSPACE")) ? NDP_OPEN_FLAG_USERSPACE : 0);
	if (p->tx == NULL) {
